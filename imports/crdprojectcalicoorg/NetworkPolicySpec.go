...
type NetworkPolicySpec struct {
	...
	Egress *[]*NetworkPolicySpecEgress `field:"optional" json:"egress" yaml:"egress"`
	// The ordered set of ingress rules.
	//
	// Each rule contains a set of packet match criteria and a corresponding action to apply.
	Ingress *[]*NetworkPolicySpecIngress `field:"optional" json:"ingress" yaml:"ingress"`
	...
	//
	// When not explicitly specified (and so the value on creation is empty or nil), Calico defaults Types according to what Ingress and Egress are present in the policy.  The default is:
